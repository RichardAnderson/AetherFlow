    ...
    public partial class XrmFakedContext : IXrmContext
    {
                ...
                if (EntityMetadata.ContainsKey(record.LogicalName))
                {
                    ...
                    foreach (var key in entityMetadata.Keys)
                    {
                        if (record.KeyAttributes.Keys.Count == key.KeyAttributes.Length && key.KeyAttributes.All(x => record.KeyAttributes.Keys.Contains(x)))
                        {
                            ...
                            if (validate)
                            {
                                new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"{record.LogicalName} with the specified Alternate Keys Does Not Exist"));
                            }
                        }
                    }
                }
        ...
        protected void UpdateEntity(Entity e)
        {
            ...
            else
            {
                ...
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"{e.LogicalName} with Id {e.Id} Does Not Exist"));
            }
        }
        ...
        protected EntityReference ResolveEntityReference(EntityReference er)
        {
            if (!Data.ContainsKey(er.LogicalName) || !Data[er.LogicalName].ContainsKey(er.Id))
            {
                ...
                {
                    throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"{er.LogicalName} With Id = {er.Id:D} Does Not Exist"));
                }
            }
            return er;
        }
        ...
        protected void DeleteEntity(EntityReference er)
        {
            ...
            else
            {
                ...
                // The entity record was not found, return a CRM-ish update error message
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"{er.LogicalName} with Id {er.Id} Does Not Exist"));
            }
        }
        #endregion
