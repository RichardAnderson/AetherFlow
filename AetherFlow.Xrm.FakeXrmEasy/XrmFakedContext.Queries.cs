...
using AetherFlow.Xrm.FakeXrmEasy.Extensions;
using AetherFlow.Xrm.FakeXrmEasy.Extensions.FetchXml;
using AetherFlow.Xrm.FakeXrmEasy.Models;
...
namespace AetherFlow.Xrm.FakeXrmEasy
{
    public partial class XrmFakedContext : IXrmContext
    {
        ...
        protected static void ValidateAliases(QueryExpression qe, XrmFakedContext context, ConditionExpression condition)
        {
            ...
            if (matches > 1)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"Table {condition.EntityName} is not unique amongst all top-level table and join aliases"));
            }
            else if (matches == 0)
            {
                ...
                if (matches > 1)
                {
                    throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"There's more than one LinkEntity expressions with name={condition.EntityName}"));
                }
                else if (matches == 0)
                {
                    if (condition.EntityName == qe.EntityName) return;
                    throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason($"LinkEntity with name or alias {condition.EntityName} is not found"));
                }
