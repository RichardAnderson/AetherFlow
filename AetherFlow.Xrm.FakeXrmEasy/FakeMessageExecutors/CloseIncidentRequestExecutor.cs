...
namespace AetherFlow.Xrm.FakeXrmEasy.FakeMessageExecutors
{
    public class CloseIncidentRequestExecutor : IFakeMessageExecutor
    {
        ...
        public OrganizationResponse Execute(OrganizationRequest request, XrmFakedContext ctx)
        {
            ...
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Cannot close incident without incident resolution."));
            }

            var status = closeIncidentRequest.Status;
            if (status == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Cannot close incident without status."));
            }
                ...
                ctx.Data[IncidentLogicalName].Values.SingleOrDefault(p => p.Id == incidentId.Id) == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(),
                    new FaultReason($"Incident with id {incidentId.Id} not found."));
            }
