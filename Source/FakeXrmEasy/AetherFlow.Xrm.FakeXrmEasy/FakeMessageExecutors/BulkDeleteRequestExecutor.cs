...
namespace AetherFlow.Xrm.FakeXrmEasy.FakeMessageExecutors
{
    public class BulkDeleteRequestExecutor : IFakeMessageExecutor
    {
        ...
        public OrganizationResponse Execute(OrganizationRequest request, XrmFakedContext ctx)
        {
            ...
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not Bulk delete without JobName"));
            }
            if (bulkDeleteRequest.QuerySet == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not Bulk delete without QuerySet"));
            }
            if (bulkDeleteRequest.CCRecipients == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not Bulk delete without CCRecipients"));
            }
            if (bulkDeleteRequest.ToRecipients == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not Bulk delete without ToRecipients"));
            }
