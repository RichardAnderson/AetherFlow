...
namespace AetherFlow.Xrm.FakeXrmEasy.FakeMessageExecutors
{
    public class AddToQueueRequestExecutor : IFakeMessageExecutor
    {
        ...
        public OrganizationResponse Execute(OrganizationRequest request, XrmFakedContext ctx)
        {
            ...
            if (target == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not add to queue without target"));
            }

            if (destinationQueueId == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("Can not add to queue without destination queue"));
            }
