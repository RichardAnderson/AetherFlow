...
namespace AetherFlow.Xrm.FakeXrmEasy.FakeMessageExecutors
{
    public class RemoveMembersTeamRequestExecutor : IFakeMessageExecutor
    {
        ...
        public OrganizationResponse Execute(OrganizationRequest request, XrmFakedContext ctx)
        {
            ...
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("TeamId parameter is required"));
            }

            if (req.MemberIds == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason("MemberIds parameter is required"));
            }
            ...
            if (team == null)
            {
                throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason(
                    $"Team with Id {req.TeamId.ToString()} wasn't found"));
            }
            ...
            foreach (var memberId in req.MemberIds)
            {
                ...
                {
                    throw new FaultException<OrganizationServiceFault>(new OrganizationServiceFault(), new FaultReason(
                        $"SystemUser with Id {memberId.ToString()} wasn't found"));
                }
